...
clear;
circ = 'oscillator';
variant = 3;  % 1 - Colpitts1 (oscillator frequency of aprox. 30 kHz)
              % 3 - Colpitts3 (oscillator frequency of aprox. 11 kHz)
...
% --- Input signal -------------------------------------------------------------
input_sig = 'const';
V_const = 12;    % Value of the input signal
t_final = 5e-3;  % end time
...
[A, B, G, H, D, E, M, L, N] = oscillator_matrices(variant);

% Transistor model: 2N4124. The values were obtained as shown here:
% https://chatgpt.com/share/e/69aa400a-fc38-8005-98ba-9b3615fa9064
VT = 25.9e-3; % V
Is = 1.0e-14; % A
beta_F = 350;
beta_R = 1;
...
%% Save the results
if(save_results)
    gen_results(circ, variant, input_sig, 0, 0, t_final, V_const,...
...
% To obtain the matrices corresponding to the Colpitts oscillator circuit
function [A, B, G, H, D, E, M, L, N] = oscillator_matrices(variant)
    R = 1e-3;  % Internal resistance of the inductors (considered in series)
    
    if variant == 1
        % Colpitts 1
        R1 = 1.5e3;  R2 = 1.2e3;  R3 = 1.5e3;
        Cap1 = 10e-9; Cap2 = 100e-9; Cap3 = 0.1e-6; Cap4 = 1e-6;
        L1 = 5e-3; L2 = 220e-3;
    else
        % Colpitts 3
        R1 = 4.7e3;  R2 = 1.5e3;  R3 = 10e3;
        Cap1 = 24e-9; Cap2 = 240e-9; Cap3 = 0.1e-6; Cap4 = 1e-6;
        L1 = 10e-3; L2 = 300e-3;
    end

    A = [   0,     0,                          0, 0, -1/(Cap1), 1/Cap1;
            0,     0, -(1/(R1*Cap2)+1/(R3*Cap2)), 0,    1/Cap2,      0;
            0,     0,               -1/(R2*Cap3), 0,         0,      0;
            0,     0, -(1/(R1*Cap4)+1/(R3*Cap4)), 0,         0,      0;
         1/L1, -1/L1,                          0, 0,     -R/L1,      0;
            0,     0,                      -1/L2, 0,         0,  -R/L2];

    B = [0; 1/(R3*Cap2); 0; 1/(R3*Cap4); 0; 1/L2];

    G = [                         0,    0,       0, -1/Cap1;
         -(1/(R1*Cap2)+1/(R3*Cap2)),    0, -1/Cap2,  1/Cap2;
                                  0,    0,  1/Cap3,       0;
         -(1/(R1*Cap4)+1/(R3*Cap4)),    0, -1/Cap4,  1/Cap4;
                                  0,    0,       0,       0;
                              -1/L2, 1/L2,       0,       0];
